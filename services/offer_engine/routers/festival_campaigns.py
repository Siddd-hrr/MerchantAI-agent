...
router = APIRouter(prefix="/offers", tags=["offers-festival-campaigns"])


class FestivalCampaignCreateRequest(BaseModel):
...
@router.post("/festival-campaign")
async def create_festival_campaign(
    payload: FestivalCampaignCreateRequest,
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create festival campaign: {exc}") from exc
